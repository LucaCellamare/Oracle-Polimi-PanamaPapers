...
#############################
#############################
#def foo_bar2(X_train,u):
        #name_=X_train.name[X_train.phone=='missing']
        #X_train.loc[X_train=='missing','phone']=u.loc[name_,'phone']
        #riga_nome=X_train.loc[u.index,'name']
        #print(riga_nome)
        #colonna=u.column
    #return X_train
...
def func(X_train,df_new,cols):
    ...
    for row_index,row in X_train_.iterrows():
        for j in range(len(cols)):
            val=X_train_.get_value(row_index,cols[j])
            if val=='missing':
                X_train_.set_value(row_index,cols[j],df_new[cols[j]].loc[X_train_.name[row_index]])
    return X_train_

@timeit
def func2(X_train,df_new,cols):
    X_train_2=X_train.copy()
    for row_index,row in X_train_2.iterrows():
        for j in range(len(cols)):
            val=X_train_2.get_value(row_index,cols[j])
            if val=='missing':
                X_train_2.set_value(row_index,cols[j],df_new[cols[j]].loc[X_train_2.phone[row_index]])
    return X_train_2
...
if __name__ == "__main__":
    ...
    n_m_n=X_train.loc[ (X_train['name'])=="missing", 'name' ].shape[0]
    n_m_p=X_train.loc[ (X_train['phone'])=="missing", 'phone' ].shape[0]
    n_m_a=X_train.loc[ (X_train['address'])=="missing", 'address' ].shape[0]
    n_m_e=X_train.loc[ (X_train['email'])=="missing", 'email' ].shape[0]
    print(f"There are {n_m_n} missing-values in column name")
    print(f"There are {n_m_p} missing-values in column phone")
    print(f"There are {n_m_a} missing-values in column address")
    ...
    X_train_pulito=X_train_pulito.applymap(lambda x : np.NaN if x=='missing' else x)
    X_train_pulito=X_train_pulito.applymap(lambda x : np.NaN if x==['missing'] else x)
    X_train_pulito=X_train_pulito.applymap(lambda x: [y for y in x if y!="missing"] if isinstance(x, list) else x)
    print(X_train_pulito.name[0:10])
    print(X_train_pulito.phone[0:10])
    print(X_train_pulito.address[0:10])
    print(X_train_pulito.email[0:10])
    
    #num_miss_name1 = X_train_pulito.loc[(X_train_pulito['name'])==np.NaN, 'name' ].shape[0]
    #num_miss_phone1 = X_train_pulito.loc[(X_train_pulito['phone'])==np.NaN, 'phone' ].shape[0]
    #num_miss_address1 = X_train_pulito.loc[X_train_pulito['address']==np.NaN, 'address' ].shape[0]
    #num_miss_email1 = X_train_pulito.loc[X_train_pulito['email']==np.NaN , 'email' ].shape[0]
    #print(f"There are {num_miss_name1} missing-values in column name")
    #print(f"There are {num_miss_phone1} missing-values in column phone")
    #print(f"There are {num_miss_address1} missing-values in column address")
    #print(f"There are {num_miss_email1} missing-values in column email")
    ...
    X_train=X_train[['phone','name','address','email']]
    X_train_pulito2=func2(X_train,df_new2,cols2)
    #X_train_pulito2=X_train_pulito2.applymap(lambda x: [y for y in x if y!="missing"] if isinstance(x, list) else x)
    X_train_pulito2=X_train_pulito2.applymap(lambda x : np.NaN if x=='missing' else x)
    X_train_pulito2=X_train_pulito2.applymap(lambda x : np.NaN if x==['missing'] else x)
    X_train_pulito2=X_train_pulito2.applymap(lambda x: [y for y in x if y!="missing"] if isinstance(x, list) else x)
    X_train_pulito2=X_train_pulito2[['name','phone','address','email']]
    print(X_train_pulito2.name[0:10])
    print(X_train_pulito2.phone[0:10])
    print(X_train_pulito2.address[0:10])
    print(X_train_pulito2.email[0:10])


    num_miss_name = X_train_pulito2.loc[(pd.isna(X_train_pulito2['name'])), 'name' ].shape[0]
    num_miss_phone = X_train_pulito2.loc[(pd.isna(X_train_pulito2['phone'])), 'phone' ].shape[0]
    num_miss_address = X_train_pulito2.loc[(pd.isna(X_train_pulito2['address'])) , 'address' ].shape[0]
    num_miss_email = X_train_pulito2.loc[(pd.isna(X_train_pulito2['email'])), 'email' ].shape[0]
    
    # summarize the num_nan_b
    #print(df)
    #print(' ')
    print(f"There are {num_miss_name} missing-values in column name")
    print(f"There are {num_miss_phone} missing-values in column phone")
    print(f"There are {num_miss_address} missing-values in column address")
    print(f"There are {num_miss_email} missing-values in column email")

    X_train_pulito=X_train_pulito[['name','phone','address','email']]
    X_train_pulito2=X_train_pulito2[['name','phone','address','email']]
    X_train_pulito = X_train_pulito.sort_index()
    X_train_pulito2 = X_train_pulito2.sort_index()
    X_train_pulito.update(X_train_pulito2)

    print(X_train_pulito)
    num_miss_name2 = X_train_pulito.loc[(pd.isna(X_train_pulito['name'])), 'name' ].shape[0]
    num_miss_phone2 = X_train_pulito.loc[(pd.isna(X_train_pulito['phone'])), 'phone' ].shape[0]
    num_miss_address2 = X_train_pulito.loc[(pd.isna(X_train_pulito['address'])) , 'address' ].shape[0]
    num_miss_email2 = X_train_pulito.loc[(pd.isna(X_train_pulito['email'])), 'email' ].shape[0]
    
    # summarize the num_nan_b
    #print(df)
    #print(' ')
    print(f"There are {num_miss_name2} missing-values in column name")
    print(f"There are {num_miss_phone2} missing-values in column phone")
    print(f"There are {num_miss_address2} missing-values in column address")
    print(f"There are {num_miss_email2} missing-values in column email")
    #for row_index, row in X_train.iterrows():
        #for j in range(len(cols)):
            #val=X_train.get_value(row_index,cols[j+1])
            #if val=='missing':
                #X_train.set_value(row_index,cols[j+1],df_new[cols[j]].loc[X_train.name[row_index]]) 

        #X_train.apply(func)
            #X_train.iloc[i,j+1]=df_new[cols[j]].loc[X_train.name[i]]
            #print(X_train.iloc[i,j+1])
            #temp=df_new.loc[X_train.name[i]].iloc[]
            #print(temp)
    #X_train=X_test.sort_values('name')
    #X_train['phone_2']=X_train.groupby('name')['phone'].ffill(limit=3)
    #print(X_train)
