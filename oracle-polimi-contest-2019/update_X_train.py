...
def func3(X_train,df_new,cols):
    ...
    for row_index,row in X_train_3.iterrows():
        for j in range(len(cols)):
            val=X_train_3.get_value(row_index,cols[j])
            if val=='missing':
                X_train_3.set_value(row_index,cols[j],df_new[cols[j]].loc[X_train_3.address[row_index]])
    return X_train_3

@timeit
def func4(X_train,df_new,cols):
    X_train_4=X_train.copy()
    for row_index,row in X_train_4.iterrows():
        for j in range(len(cols)):
            val=X_train_4.get_value(row_index,cols[j])
            if val=='missing':
                X_train_4.set_value(row_index,cols[j],df_new[cols[j]].loc[X_train_4.email[row_index]])
    return X_train_4
if __name__ == "__main__":
    ...
    X_train=X_train[['address','name','phone','email']]
    X_train_pulito3=func3(X_train,df_new3,cols3)
    X_train_pulito3=X_train_pulito3.applymap(lambda x : np.NaN if x=='missing' else x)
    X_train_pulito3=X_train_pulito3.applymap(lambda x : np.NaN if x==['missing'] else x)
    ...
    X_train=X_train[['email','name','phone','address']]
    X_train_pulito4=func4(X_train,df_new4,cols4)
    X_train_pulito4=X_train_pulito4.applymap(lambda x : np.NaN if x=='missing' else x)
    X_train_pulito4=X_train_pulito4.applymap(lambda x : np.NaN if x==['missing'] else x)
    ...
    X_train_pulito3=X_train_pulito3[['name','phone','address','email']]
    X_train_pulito4=X_train_pulito4[['name','phone','address','email']]
    X_train_finale=X_train_pulito.copy()
    X_train_finale=X_train_finale[['name','phone','address','email']]
    X_train_pulito = X_train_pulito.sort_index()
    X_train_pulito2 = X_train_pulito2.sort_index()
    X_train_pulito3 = X_train_pulito3.sort_index()
    X_train_pulito4 = X_train_pulito4.sort_index()
    X_train_finale = X_train_finale.sort_index()
    X_train_finale.update(X_train_pulito2)
    X_train_finale.update(X_train_pulito3)
    X_train_finale.update(X_train_pulito4)
    print(X_train_pulito2)
    print(X_train_pulito3)
    print(X_train_pulito4)
    print(X_train_finale)

    num_miss_name_f = X_train_finale.loc[(pd.isna(X_train_finale['name'])), 'name' ].shape[0]
    num_miss_phone_f = X_train_finale.loc[(pd.isna(X_train_finale['phone'])), 'phone' ].shape[0]
    num_miss_address_f = X_train_finale.loc[(pd.isna(X_train_finale['address'])),'address' ].shape[0]
    num_miss_email_f = X_train_finale.loc[(pd.isna(X_train_finale['email'])),'email' ].shape[0]
    
 
    print(f"There are {num_miss_name_f} missing-values in column name final")
    print(f"There are {num_miss_phone_f} missing-values in column phone final")
    print(f"There are {num_miss_address_f} missing-values in column address final")
    print(f"There are {num_miss_email_f} missing-values in column email final")
